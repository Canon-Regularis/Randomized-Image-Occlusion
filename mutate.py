...
MUTATIONS: list[Mutation] = [
    ...
    _m(READER, "the reader does not swap header and back extra",
       ...
       "            back_extra=fields.get(spec.header_field, \"\"),"),
    ...
    _m(FACTORY, "header and back extra are not swapped",
       ...
       "            spec.header_field: back_extra,\n            spec.back_extra_field: header,"),
    ...
    _m(RENDER, "the once-per-show guard holds", "      if (ran) return;\n      ran = true;", "      if (false) return;\n      ran = true;"),
    _m(RENDER, "the guard is armed after the first pass", "      if (ran) return;\n      ran = true;", "      if (ran) return;\n      ran = false;"),
    ...
    _m(RENDER, "an image with no size counts as not ready",
       "    if (!img || !box.width || !box.height) {", "    if (!img) {"),
    ...
    _m(RENDER, "the question side shows the prompt, not the answer",
       ...
       "          drawBox(svg, layout.centers[ci], layout.targets[ci], layout.targets[ci].label, cfg, true, undefined, layout.targets[ci].label);"),
    _m(RENDER, "a locate marker is drawn without an arrow",
       ...
       "          drawBox(svg, layout.centers[ci], layout.targets[ci], currentStructure().label, cfg, true);"),
    ...
    _m(RENDER, "a reverse question side withholds every context arrow",
    ...
    _m(RENDER, "the arrow tail never passes the target",
    ...
    _m(RENDER, "decoy dots are drawn",
       ...
       "      // Decoy dots (all markers) force the learner to follow the arrow to the\n      // right one instead of recognising a lone dot.\n      if (false) {"),
    _m(RENDER, "context mode draws decoy dots",
    ...
    _m(RENDER, "the lone target dot is drawn",
       ...
       "      } else if (false) {\n        drawDot(svg, active);\n      }\n      var center = placeCenter(rng, stage, active, cfg);"),
    _m(RENDER, "context mode draws the lone target dot",
       "      } else if (targetDotVisible(cfg, isReverse, back)) {\n        drawDot(svg, active);\n      }\n      for (var b = 0;",
       "      } else if (false) {\n        drawDot(svg, active);\n      }\n      for (var b = 0;"),
