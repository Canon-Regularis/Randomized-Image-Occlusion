...
RUNNER = Target("src/randomized_occlusion/ops/runner.py", ("tests/test_ops_runner.py",))
...
MUTATIONS: list[Mutation] = [
    ...
    _m(STRUCTSET, "a mark past the ceiling is discarded, not obeyed",
       "        if self.next_ordinal < floor or self.next_ordinal > MAX_ORDINAL:",
       "        if self.next_ordinal < floor:"),
    _m(STRUCTSET, "the ceiling is the last usable ordinal, not one past it",
       "        if self.next_ordinal < floor or self.next_ordinal > MAX_ORDINAL:",
       "        if self.next_ordinal < floor or self.next_ordinal > MAX_ORDINAL + 1:"),
    _m(STRUCTSET, "too many structures is reported without listing them",
       '                f"this note has {len(ordinals)} structures, numbered up to "',
       '                f"structure ordinals must be at most {MAX_ORDINAL}; got {ordinals}"'),
    _m(STRUCTSET, "a long ordinal list is summarised",
       "    if len(ordinals) <= limit:", "    if True:"),
    ...
    _m(INSTALLER, "a template the user edited is left alone",
       "        if self._is_customised(existing, stored_css):", "        if False:"),
    _m(INSTALLER, "leaving it alone is reported, not passed off as an update",
       "            return InstallResult.CUSTOMISED",
       "            return InstallResult.UPDATED"),
    _m(INSTALLER, "a customised notetype still gets its field migration",
       "            if fields_changed or collapse_changed or sort_changed:",
       "            if False:"),
    _m(INSTALLER, "the stored strings are what gets written back",
       '                    front=first.get("qfmt", ""),',
       "                    front=template.front,"),
    _m(INSTALLER, "the stored back is what gets written back",
       '                    back=first.get("afmt", ""),',
       "                    back=template.back,"),
    _m(INSTALLER, "the stored css is what gets written back",
       "                    css=stored_css,",
       "                    css=template.css,"),
    _m(INSTALLER, "a notetype carrying no fingerprint counts as ours",
       "        if marker is None:", "        if marker is not None:"),
    _m(INSTALLER, "a renamed field is refused, not repaired",
       "        if self.missing_required_fields(existing):", "        if False:"),
    _m(INSTALLER, "the refusal is reported, not passed off as no change",
       "            return InstallResult.FIELDS_MISSING",
       "            return InstallResult.UNCHANGED"),
    _m(INSTALLER, "every required field is checked, not just the first",
       "        return tuple(name for name in self._spec.required_fields if name not in names)",
       "        return ()"),
    _m(SPEC, "every field the note type was created with is required",
       '    required_fields=("Image", "Structures", "Ordinals", "Header", "Back Extra"),',
       '    required_fields=("Image",),'),
    _m(SPEC, "a field a later version added is not required",
       '    required_fields=("Image", "Structures", "Ordinals", "Header", "Back Extra"),',
       '    required_fields=("Image", "Structures", "Ordinals", "Header", "Back Extra", "TypeAnswer"),'),
    _m(SPEC, "a spec demanding a field it never creates is refused",
       "        if unknown:", "        if False:"),
    _m(RUNNER, "the note type is installed before the note is built",
       "    if installer.ensure_installed(render_config) is InstallResult.FIELDS_MISSING:",
       "    if None is InstallResult.FIELDS_MISSING:"),
    _m(RUNNER, "a refused install stops the op",
       "    if installer.ensure_installed(render_config) is InstallResult.FIELDS_MISSING:",
       "    if installer.ensure_installed(render_config) is InstallResult.UNCHANGED:"),
    _m(RUNNER, "the refusal names the field that is gone",
       "        missing = installer.missing_required_fields(",
       "        missing = tuple(spec.required_fields) or installer.missing_required_fields("),
    _m(TEMPLATES, "the marker line is dropped before hashing",
       '    return _FINGERPRINT_LINE_RE.sub("", css or "", count=1)',
       '    return css or ""'),
    _m(TEMPLATES, "a stored template is hashed without its own marker",
       "    return _hash(front, back, strip_fingerprint(css))",
       "    return _hash(front, back, css)"),
    ...
    _m(BUILD, "entries are root-relative", "arcname = relative.as_posix()", 'arcname = "sub/" + relative.as_posix()'),
    _m(BUILD, "runtime state under user_files is excluded",
       "        return relative.parts == (USER_FILES_DIR, USER_FILES_KEEP)",
       "        return True"),
    _m(BUILD, "the user_files marker itself still ships",
       "        return relative.parts == (USER_FILES_DIR, USER_FILES_KEEP)",
       "        return False"),
    ...
    _m(RENDER, "an ordinal that matches nothing falls back to the first structure",
       "    if (activeIndex < 0) activeIndex = 0;", ""),
    _m(RENDER, "the active ordinal is looked up, not subtracted from",
       "      if (Number(structures[i].ord) === ordinal) return i;",
       "      if (i === ordinal - 1) return i;"),
    _m(RENDER, "a card we cannot draw is left blank, not drawn as card 1",
       "    if (indexOfOrdinal(structures, activeOrdinal) < 0) {",
       "    if (false) {"),
    _m(RENDER, "a card with no active cloze reads as no card, not as card 1",
       "    return 0;", "    return 1;"),
    _m(RENDER, "an undrawable card takes the overlay down",
    ...
    _m(SHORTCUTS, "the keys are appended, not substituted",
       ...
       "        shortcuts[:] = [(key, handler) for key in keys]"),
    _m(SHORTCUTS, "the caller chooses the key spellings",
       "    keys: tuple[Any, ...] = _KEYS,", "    keys: tuple[Any, ...] = (),"),
    ...
    _m(SHORTCUTS, "a broken note falls through rather than raising",
       "            intercept = False\n        if not intercept:",
       "            raise\n        if not intercept:"),
    _m(SHORTCUTS, "a note we cannot read is reported once, not silently",
       "            if not reported[0]:", "            if False:"),
    _m(SHORTCUTS, "the report is latched so it cannot spam",
       "                reported[0] = True", "                pass"),
    _m(SHORTCUTS, "a stale callback does not flip a card that moved on",
